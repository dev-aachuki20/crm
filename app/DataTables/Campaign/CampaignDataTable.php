        ...
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($data) {
                return '<div class="edit-delete">'
                    .(auth()->user()->can('compaign_edit') ?
                        '<button type="button" id="user" class="edit" campaign_id=' . $data->id . ' onclick="editForm(' . $data->id . ')">
                    ...
                    : '')
                    .(auth()->user()->can('compaign_delete') ? 
                        '<button type="button" class="delete" data-user_id=' . $data->id . ' onclick="deleteCompaign(' . $data->id . ')">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
